from flask import Flask ,request , redirect , url_for
import random
from datetime import datetime
import json


randomnumberlist = [0]
tablenumber = "A"
app = Flask(__name__)

counter = {}
hein = {}
ordereditemswithtime = {}


def newitemadder(countername,image):
    
    templateforItem = f"""<p>{countername}-nocount-</p>
    <img src="{image}">
    <p>Type in the amount you want</p>
    <form action="/texted" method="post">
      <input type="number" name="{countername}">
    </form>
    <div style="display:flex; gap:10px;">
    <form action="/+clicked" method="post">
    <button type="submit" value="{countername}" name="plus">+</button>
    </form>
    <form action="/-clicked" method="post">
    <button type="submit" value="{countername}" name="minus">-</button>
    </form>
    </div>"""
   
    hein[countername] = templateforItem
    counter[countername] = 0

def pageopener():
    global page
    page = ""
    f = open("/workspaces/Hhh/test.html","r")
    page = f.read()
    for key in hein:
        page += hein[key]
    for identifier , value in hein.items():
        name = identifier
        page = page.replace(f"{name}-nocount-",str(counter[name]))
    page += """ <form action="/submit" method="post">
    <button type="submit"  name="data" value= "True">submit</button>
    </form>"""
    f.close()
    return page


newitemadder("apple","data:image/jpeg;base64,")
newitemadder("ornage","data:image/jpeg;base64,")
newitemadder("banana","data:image/jpeg;base64,")


@app.route("/")
def index():
    pageopener()
    return page
    
@app.route(f"/+clicked",methods = ["post"])
def clickedplus():
    form = request.form
    namae = form["plus"]
    if counter[namae] < 10:
        counter[namae] += 1
        
    
    pageopener()
    return page

@app.route(f"/-clicked",methods = ["post"])
def clickedmius():
    form = request.form
    namae = form["minus"]
    if counter[namae] > 0:
        counter[namae] -= 1
    
    pageopener()
    return page

@app.route("/texted",methods = ["post"])
def textt():
    form = request.form
    for key, value in form.items():
        if value == "":
            pass
        else:
            value = int(value)
        
            if value <= 10 and value > 0:
                counter[key] = value
        
        
    pageopener()
    return page

@app.route("/submit",methods = ["post"])
def submit():
    
   
    
    

    page1 = ""
    f = open("/workspaces/Hhh/ordercheck.html","r")
    page1 = f.read()
    for key , value in counter.items():
        page1 += f"<p>{key} = {value}</p>"
    page1 += f"""<form action="/finalpage" method="post">
    <button type="submit" value="" name="">Ok</button>
    </form>
    <form action="/return" method="post">
    <button type="submit" value="" name="">NotOk</button>
    </form>
    """
    f.close()
    
    return page1

@app.route("/finalpage",methods = ["post"])
def final():
    global counter
    num = random.randint(1,10000)
    if num not in randomnumberlist:
        randomnumberlist.append(num)
    currenttime = datetime.now().time()
    ordereditemswithtime[f"{randomnumberlist[-1]}@@@{counter}"] = currenttime
    for key , value in ordereditemswithtime.items():
        newkey = key.split("@@@")
        orderrandomNumber = newkey[0]
        ordersitems = newkey[1]
        neworderItems = ordersitems.split(",")
        for items in neworderItems:
            newitems = items.split(": ")
            if "'ornage'" in newitems:
                print("Yes")
            else:
                print("No")
            print(newitems)

        print(ordersitems)
    print(ordereditemswithtime)
    f = open("/workspaces/Hhh/aaa.txt","a")
    page1 = f.write(f"""
[
    Table = {tablenumber},
    order = {counter}
    time = {currenttime} 
    number = {randomnumberlist[-1]}
]
                    """)
    for key, value in counter.items():
        counter[key] = 0
        
    return redirect("/")


@app.route("/return",methods = ["post"])
def retrun():
    return redirect("/")

app.run(host="0.0.0.0",port=50001)
